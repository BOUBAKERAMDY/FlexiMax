...
\begin{resultbox}[{Source : \code{06\_timeseries/timeseries\_clustering\_multi.ipynb} --- bâtiment 10672-0, premier des 100 bâtiments tout-électriques du panel}]
Chaque journée (pas horaire, 24 valeurs) est normalisée de deux façons concurrentes~: un z-score
\emph{intra-jour} qui ne garde que la \og forme \fg{} du profil (où se situent les pics, quel que
soit le niveau), et un \code{StandardScaler} \emph{global} qui garde surtout le niveau
d'\og amplitude \fg{} du jour. Un K-Means est ajusté séparément sur chaque représentation, avec $k$
choisi par méthode du coude puis vérifié par score de silhouette. Les deux normalisations donnent
des résultats très différents~: la forme retient $k=6$ (tailles 63/44/133/55/38/32) mais avec une
silhouette faible et décroissante (0,34 à $k=3$, 0,25 à $k=6$) --- les formes de profil se
chevauchent beaucoup ; l'amplitude retient $k=5$ (tailles 22/231/11/71/30) avec une silhouette
nettement meilleure (0,58 à $k=2$, encore 0,44 à $k=5$). Pour ce logement, les journées se
distinguent donc bien plus par leur \emph{niveau} de consommation que par la \emph{forme} de leur
courbe horaire --- un grand cluster majoritaire (231 jours) porte le régime courant, entouré de
clusters minoritaires qui captent les jours extrêmes.
\end{resultbox}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{timeseries_multi_calendrier_composition_forme.png}
  \caption{Composition des 6 clusters de \og forme \fg{} par répartition semaine/weekend et par
  mois --- bâtiment 10672-0.}
  \label{fig:clustering-jours-types-calendrier}
\end{figure}

La composition calendaire des clusters de forme (figure~\ref{fig:clustering-jours-types-calendrier})
confirme ce diagnostic~: la répartition semaine/weekend reste proche de 70--78\% dans tous les
clusters, c'est-à-dire proche du ratio naturel des jours ouvrés (5/7~$\approx$~71\%) --- le
weekend n'isole pas un cluster à part. En revanche la répartition mensuelle est très marquée
(clusters 0 et 3~: hiver ; clusters 4 et 5~: printemps ; clusters 1 et 2~: été), ce qui confirme
à l'échelle du profil horaire ce que le chapitre~\ref{ch:exploration} établit à l'échelle
annuelle~: le climat structure la consommation avant le rythme hebdomadaire.
...
décrit par sa forme (réduite par PCA --- une seule composante suffit à expliquer 90\% de la
variance de forme sur l'ensemble du panel, signe d'un mode de variation dominant), son amplitude
(log, centrée-réduite \emph{par bâtiment} pour ne pas confondre \og jour atypique \fg{} et
\og gros bâtiment \fg{}) et 12 variables météo (moyenne/min/max de température, humidité relative,
température humide, ratio d'humidité), soit 23 dimensions au total. La méthode du coude retient
...
comportements que de régimes bien séparés. L'ajustement final du clustering multivarié sur
l'ensemble du panel n'a pas été mené à terme dans le temps du stage~: cette section pose le cadre
méthodologique (extraction, réduction, choix de $k$) plutôt qu'une typologie de jours types
validée à l'échelle du parc.
\end{resultbox}
...
régression commune. Le split train/val/test (70/15/15) se fait \emph{par bâtiment}, pas par
fenêtre temporelle~: le modèle est testé sur des bâtiments jamais vus, condition plus proche d'un
usage en diagnostic énergétique qu'un split temporel intra-bâtiment.

\textbf{Partie A --- prévision à t+1 (fenêtre de 24h).} Avec 611\,520 fenêtres d'entraînement (70
bâtiments $\times$ 8\,736 fenêtres) et un GRU à 32 unités, le modèle hybride bat la baseline de
persistance (dernière heure observée)~: $R^2=0{,}877$ contre $0{,}827$, RMSE $=1{,}00$~kWh contre
$1{,}19$~kWh (-16\%). Le gain est net mais mesuré~: la persistance est déjà un concurrent sérieux à
l'échelle horaire, la dynamique de consommation étant fortement autocorrélée d'une heure à l'autre.

\textbf{Partie B --- extrapolation du total annuel (90 premiers jours observés).} Avec seulement 70
exemples d'entraînement (une observation par bâtiment) et un réseau volontairement réduit (GRU 16
unités, dropout 0,3), le modèle hybride explique $R^2=0{,}803$ de la variance du total annuel
(RMSE $=5\,607$~kWh contre $12\,637$~kWh pour une baseline naïve --- la moyenne du train, -56\%).
Le signal est réel mais évalué sur seulement 15 bâtiments de test~: à confirmer sur un panel plus
large avant toute conclusion définitive, comme le note explicitement le notebook source.
\end{resultbox}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Partie A --- prévision t+1 (24h).}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{rnn_mlp_partieB_metrics_bar.png}
    \caption{Partie B --- extrapolation annuelle (90j).}
  \end{subfigure}
  \caption{RMSE et MAE --- modèle hybride GRU+MLP contre baseline, pour les deux horizons de
  prédiction.}
  \label{fig:rnn-mlp-metriques}
\end{figure}
